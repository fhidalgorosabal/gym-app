...
@Component({
  ...
  template: `
    <div class="p-4">
      <h2 class="text-xl font-bold text-gray-800">Configurar Rutina</h2>
      <p class="mt-1 text-sm text-gray-500">Activa los días que entrenas y configura los ejercicios de cada uno.</p>
          ...
          <li class="flex items-center justify-between rounded-xl border bg-white px-4 py-3 shadow-sm">
            ...
            <div class="flex items-center gap-3">
              <button
                ...
                [attr.aria-label]="day.is_active ? 'Desactivar ' + day.name : 'Activar ' + day.name"
                class="relative h-6 w-11 rounded-full transition-colors duration-200"
                [class.bg-indigo-600]="day.is_active"
            ...
            @if (day.is_active) {
              <a
                ...
                class="flex items-center gap-1 rounded-lg bg-indigo-50 px-3 py-1.5 text-xs font-medium text-indigo-600 transition-colors hover:bg-indigo-100"
              >
